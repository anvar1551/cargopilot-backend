...
function box(doc, x, y, w, h, lw = 1.4) {
    doc.save();
    doc.lineWidth(lw).rect(x, y, w, h).stroke();
    doc.restore();
}
function vline(doc, x, y, h, lw = 1.2) {
    doc.save();
    doc
        .lineWidth(lw)
        .moveTo(x, y)
...
function hline(doc, x, y, w, lw = 1.2) {
    doc.save();
    doc
        .lineWidth(lw)
        .moveTo(x, y)
...
async function generateInvoicePDF(data) {
    const outputDir = path_1.default.resolve("invoices");
    ensureDir(outputDir);
    const filePath = path_1.default.join(outputDir, `${data.invoiceId}.pdf`);
    const doc = new pdfkit_1.default({ size: "A4", margin: 20, layout: "portrait" });
    ...
    fitText(doc, `Date: ${data.createdAt.toLocaleDateString("en-DE")}`, W - 120, y + 60, 100, 12, { align: "right" });
    y += headerH + 15;
    ...
    doc.font("Helvetica").fontSize(10);
    fitText(doc, `Email: ${data.customerEmail}`, m + 15, y + 32, W - qrW - 50, 12);
    ...
    const qrBuffer = await qrcode_1.default.toBuffer(data.orderId, { margin: 1, scale: 4 });
    ...
    fitText(doc, `€${(data.amount * 0.9).toFixed(2)}`, col2 + W * 0.2, y + 36, W * 0.15, 11, { align: "right" });
    fitText(doc, "VAT (7%):", col2, y + 52, W * 0.25, 11);
    fitText(doc, `€${(data.amount * 0.07).toFixed(2)}`, col2 + W * 0.2, y + 52, W * 0.15, 11, { align: "right" });
    hline(doc, col2, y + 68, W * 0.35);
    doc.font("Helvetica-Bold").fontSize(12);
    fitText(doc, "TOTAL:", col2, y + 75, W * 0.25, 14);
    fitText(doc, `€${data.amount.toFixed(2)}`, col2 + W * 0.2, y + 75, W * 0.15, 14, {
        align: "right",
    });
    y += section2H + 15;
    // ========== SECTION 3: Footer ==========
    ...
    doc.font("Helvetica").fontSize(9);
    fitText(doc, "For support, contact us at support@cargopilot.com | +49 40 1234 5678", m + 15, y + 32, W - 30, 12, { align: "center" });
    doc.end();
