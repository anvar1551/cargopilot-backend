...
function box(doc, x, y, w, h, lw = 1.2) {
    doc.save();
    doc.lineWidth(lw).rect(x, y, w, h).stroke();
    doc.restore();
}
function vline(doc, x, y, h, lw = 1) {
    doc.save();
    doc
        .lineWidth(lw)
        .moveTo(x, y)
...
function resolveSecondCodeMode() {
    const mode = String(process.env.ORDER_LABEL_SECOND_CODE_MODE ?? process.env.ORDER_LABEL_SCAN_MODE ?? "barcode")
        .trim()
        .toLowerCase();
    return mode === "qr" ? "qr" : "barcode";
}
async function buildBarcode(code, height = 10) {
    try {
...
async function generateLabelPDF(order) {
    const outputDir = path_1.default.resolve("labels");
    ensureDir(outputDir);
    const safeParcelCode = order.parcelCode.replace(/[\\/]/g, "-");
    const filePath = path_1.default.join(outputDir, `${safeParcelCode}.pdf`);
    const doc = new pdfkit_1.default({ size: "A6", margin: 12, layout: "portrait" });
    ...
    const secondBarcode = secondCodeMode === "barcode" ? await buildBarcode(order.parcelCode, 10) : null;
    const secondQr = secondCodeMode === "qr"
        ? await qrcode_1.default.toBuffer(order.parcelCode, {
            margin: 1,
            scale: 6,
        })
        : null;
    ...
    fitText(doc, `Shipment: ${order.pieceNo}/${order.pieceTotal}`, m + 10, y + 16, 140, 16);
    if (mainBarcode) {
