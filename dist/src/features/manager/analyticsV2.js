...
function buildScopeQueueWhere(scope) {
    if (scope.role !== "warehouse" || !scope.warehouseId)
        return [];
    return [
        client_1.Prisma.sql `(
...
async function getAnalyticsSummaryV2(params) {
    ...
    return (0, analyticsV2Cache_1.getOrComputeCached)({
        namespace: "summary",
        key,
        ttlMs: Number(process.env.ANALYTICS_V2_SUMMARY_TTL_MS || 60000),
        compute: async () => {
            const now = new Date();
            ...
            const [statusDistribution, createdInRange, deliveredInRange, returnedInRange, pendingInvoicesCount, paidInvoices, codExpected, serviceChargeExpected, unpaidCodCount, unpaidServiceCount] = await Promise.all([
                ...
                prismaClient_1.default.order.count({ where: { ...scopeWhere, createdAt: { gte: rangeStart, lte: rangeEnd } } }),
                prismaClient_1.default.order.count({ where: { ...scopeWhere, status: "delivered", updatedAt: { gte: rangeStart, lte: rangeEnd } } }),
                prismaClient_1.default.order.count({ where: { ...scopeWhere, status: "returned", updatedAt: { gte: rangeStart, lte: rangeEnd } } }),
                prismaClient_1.default.invoice.count({ where: { status: "pending", createdAt: { gte: rangeStart, lte: rangeEnd }, order: scopeWhere } }),
                prismaClient_1.default.invoice.aggregate({ _sum: { amount: true }, where: { status: "paid", createdAt: { gte: rangeStart, lte: rangeEnd }, order: scopeWhere } }),
                prismaClient_1.default.order.aggregate({ _sum: { codAmount: true }, where: { ...scopeWhere, createdAt: { gte: rangeStart, lte: rangeEnd }, codAmount: { not: null } } }),
                prismaClient_1.default.order.aggregate({ _sum: { serviceCharge: true }, where: { ...scopeWhere, createdAt: { gte: rangeStart, lte: rangeEnd }, serviceCharge: { not: null } } }),
                prismaClient_1.default.order.count({ where: { ...scopeWhere, createdAt: { gte: rangeStart, lte: rangeEnd }, codAmount: { gt: 0 }, codPaidStatus: { in: UNPAID_PAID_STATUSES } } }),
                prismaClient_1.default.order.count({ where: { ...scopeWhere, createdAt: { gte: rangeStart, lte: rangeEnd }, serviceCharge: { gt: 0 }, serviceChargePaidStatus: { in: UNPAID_PAID_STATUSES } } }),
            ]);
            ...
            const [overdueOpenOrders, dueSoonOpenOrders, staleOpenOrders] = await Promise.all([
                ...
                prismaClient_1.default.order.count({ where: { ...scopeWhere, status: { in: [...ACTIVE_ORDER_STATUSES] }, expectedDeliveryAt: { gte: now, lte: dueSoonEnd } } }),
...
async function getAnalyticsTrendV2(params) {
    ...
    return (0, analyticsV2Cache_1.getOrComputeCached)({
        namespace: "trend",
        key,
        ttlMs: Number(process.env.ANALYTICS_V2_TREND_TTL_MS || 60000),
        compute: async () => {
            const now = new Date();
            const rangeStart = startOfUtcDay(subtractDays(now, rangeDays - 1));
            const rangeEnd = endOfUtcDay(now);
            const scopeWhere = buildScopeOrderWhere(params.scope);
            const scopeSql = params.scope.role === "warehouse" && params.scope.warehouseId
                ? client_1.Prisma.sql `
              AND (
...
async function getAnalyticsWarningsV2(params) {
    ...
    return (0, analyticsV2Cache_1.getOrComputeCached)({
        namespace: "warnings",
        key,
        ttlMs: Number(process.env.ANALYTICS_V2_WARNINGS_TTL_MS || 60000),
        compute: async () => {
            const now = new Date();
            const rangeStart = startOfUtcDay(subtractDays(now, rangeDays - 1));
            const rangeEnd = endOfUtcDay(now);
...
async function getAnalyticsFinanceQueueV2(params) {
    ...
    return (0, analyticsV2Cache_1.getOrComputeCached)({
        ...
        key,
        ttlMs: Number(process.env.ANALYTICS_V2_FINANCE_QUEUE_TTL_MS || 60000),
        compute: async () => {
