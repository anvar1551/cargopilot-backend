...
exports.StripeProviderAdapter = void 0;
...
function toSafeNumber(amountMinor) {
...
function mapStripeCheckoutPaymentStatus(status) {
    return status === "paid" ? "succeeded" : "pending";
}
function buildCheckoutUrls(orderId) {
    const clientBase = process.env.CHECKOUT_RETURN_BASE_URL?.trim() ||
...
function extractEventIntentId(event) {
...
class StripeProviderAdapter {
    constructor() {
        this.provider = client_1.PaymentProvider.STRIPE;
    }
    ...
    async refund(input) {
        ...
        const refund = await stripe.refunds.create({
            ...
            ...(input.reason ? { reason: "requested_by_customer" } : null),
            metadata: {
                paymentIntentId: input.intent.id,
        ...
        const mappedStatus = refund.status === "succeeded" ? "refunded" : "pending";
        return {
            status: mappedStatus,
            rawResponse: {
                refundId: refund.id,
    ...
    async verifyWebhook(input) {
        ...
        const providerPaymentId = toStringValue(toRecord(event.data.object).payment_intent) || paymentIntentIdFromEvent;
...
exports.StripeProviderAdapter = StripeProviderAdapter;
