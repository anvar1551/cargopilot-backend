...
function asEnumValue(value, allowed, fieldName) {
    ...
    throw (0, __1.orderError)(`Invalid ${fieldName}: ${asText}`, 400);
}
function parseNumber(value, fieldName, required = false) {
    if (value == null || value === "") {
        if (required)
            throw (0, __1.orderError)(`${fieldName} is required`, 400);
        return undefined;
    }
    const parsed = Number(value);
    if (!Number.isFinite(parsed)) {
        throw (0, __1.orderError)(`${fieldName} must be a number`, 400);
    }
    return parsed;
}
async function ensureOrderReadAccess(req) {
    if (!req.user) {
        throw (0, __1.orderError)("Unauthorized", 401);
    }
    await (0, identity_access_1.authorize)(req.user, "orders.read");
    return (0, identity_access_1.buildOrderScopeWhere)(req.user);
}
async function ensureOrderWriteAccess(req) {
    if (!req.user) {
        throw (0, __1.orderError)("Unauthorized", 401);
    }
    await (0, identity_access_1.authorize)(req.user, "orders.write");
}
async function assertOrderInScope(req, orderId) {
    const scopeWhere = (await ensureOrderReadAccess(req)) ?? { id: "__no_access__" };
    const order = await prismaClient_1.default.order.findFirst({
        where: { AND: [{ id: orderId }, scopeWhere] },
        select: { id: true },
    });
    if (!order) {
        throw (0, __1.orderError)("Order not found", 404);
    }
}
...
async function upsertLeg(req, res) {
    try {
        await ensureOrderWriteAccess(req);
        await assertOrderInScope(req, req.params.id);
