...
function buildOrderWhere(params, enforcedScopeWhere) {
    ...
    const and = [searchWhere, filterWhere].filter((item) => !isEmptyWhere(item));
...
const listOrders = async (params, enforcedScopeWhere, cacheScopeKey) => {
    ...
    const where = buildOrderWhere(params, enforcedScopeWhere);
    if (mode === "cursor") {
...
const listOrdersForExport = async (params, enforcedScopeWhere) => {
    const where = buildOrderWhere(params, enforcedScopeWhere);
...
const countOrdersForExport = async (params, enforcedScopeWhere) => {
    const where = buildOrderWhere(params, enforcedScopeWhere);
