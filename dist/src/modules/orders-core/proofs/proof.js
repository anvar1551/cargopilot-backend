...
exports.submitProofForActor = submitProofForActor;
exports.listOrderProofLinksForActor = listOrderProofLinksForActor;
...
async function submitProofForActor(input) {
    const { actor, orderId, body, file, forcedStage } = input;
    if (!orderId)
        throw (0, shared_1.orderError)("Missing order id", 400);
    ...
    if (!signedBy)
        throw (0, shared_1.orderError)("signedBy is required", 400);
    const signaturePaths = parseSignaturePaths(body.signaturePaths);
    if (signaturePaths.length === 0) {
        throw (0, shared_1.orderError)("signaturePaths is required", 400);
    }
    if (!file?.buffer || file.size <= 0) {
        throw (0, shared_1.orderError)("photo is required", 400);
    }
    ...
    if (!bucket)
        throw (0, shared_1.orderError)("AWS_S3_BUCKET is not configured", 500);
...
async function listOrderProofLinksForActor(input) {
    const { user, orderId, query } = input;
    if (!orderId)
        throw (0, shared_1.orderError)("Missing order id", 400);
    const scopeWhere = await (0, identity_access_1.buildOrderScopeWhere)(user);
    const order = await (0, repo_1.getOrderById)(orderId, scopeWhere);
    if (!order) {
