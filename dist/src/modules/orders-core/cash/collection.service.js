...
async function collectOrderCash(params) {
    ...
    await prismaClient_1.default.$transaction(async (tx) => {
        ...
        const actorIsWarehouseContext = Boolean(actor.warehouseId) && order.currentWarehouseId === actor.warehouseId;
...
async function handoffOrderCash(params) {
    ...
    await prismaClient_1.default.$transaction(async (tx) => {
        ...
        const actorIsAssignedDriver = order.assignedDriverId === actor.id;
        const actorIsWarehouseContext = Boolean(actor.warehouseId) &&
    ...
    const items = rows.map((row) => {
        ...
        return {
            ...
            canHandoff: row.status === client_1.CashCollectionStatus.held &&
                (hasPermission(actor, "shipment.update") || hasPermission(actor, "finance.settleCash")),
            canSettle: row.status === client_1.CashCollectionStatus.held && hasPermission(actor, "finance.settleCash"),
        };
    });
    return {
        items,
