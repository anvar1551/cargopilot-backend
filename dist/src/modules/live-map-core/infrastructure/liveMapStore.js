...
exports.readDriverIdsInViewport = readDriverIdsInViewport;
...
function getViewportGeoMetrics(viewport) {
    ...
    return { centerLng, centerLat, widthM, heightM };
}
...
async function readDriverIdsInViewport(viewport) {
    const { centerLng, centerLat, widthM, heightM } = getViewportGeoMetrics(viewport);
    const redis = await (0, redis_1.getRedisClient)();
    if (!redis)
        return [];
    try {
...
async function readDriverLocationsInViewport(viewport) {
    const ids = await readDriverIdsInViewport(viewport);
    return readDriverLocations(ids);
}
