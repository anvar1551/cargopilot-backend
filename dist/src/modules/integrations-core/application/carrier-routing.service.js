...
exports.listCarrierRoutingRulesForActor = listCarrierRoutingRulesForActor;
exports.createCarrierRoutingRuleForActor = createCarrierRoutingRuleForActor;
exports.updateCarrierRoutingRuleForActor = updateCarrierRoutingRuleForActor;
exports.deleteCarrierRoutingRuleForActor = deleteCarrierRoutingRuleForActor;
exports.resolveCarrierRoutingRuleForOrderLeg = resolveCarrierRoutingRuleForOrderLeg;
...
async function assertCompanyAccess(user, companyId, permission) {
    ...
    if (scopedIds === null || scopedIds.includes(companyId))
        return;
    const err = new Error("Forbidden for this company");
    err.statusCode = 403;
    throw err;
}
async function getRuleAccessibleOrThrow(user, ruleId, permission) {
    await (0, identity_access_1.authorize)(user, permission);
    const scopedIds = await listAccessibleCompanyIds(user);
    ...
    if (!rule) {
        const err = new Error("Carrier routing rule not found");
        err.statusCode = 404;
        throw err;
    }
    if (scopedIds !== null && !scopedIds.includes(rule.companyId)) {
        const err = new Error("Forbidden for this routing rule");
        err.statusCode = 403;
        throw err;
    }
    return rule;
...
async function assertRouteTemplateForCompany(args) {
    ...
    if (!routeTemplateId && !routeTemplateLegId)
        return;
    ...
    if (routeTemplateId) {
        ...
        if (!routeTemplate) {
            const err = new Error("routeTemplateId must belong to the same company");
            err.statusCode = 400;
            throw err;
        }
    }
    if (routeTemplateLegId) {
        ...
        if (!leg) {
            const err = new Error("routeTemplateLegId must belong to the same company and route template");
            err.statusCode = 400;
            throw err;
        }
    }
...
async function assertCarrierProviderForCompany(args) {
    ...
    if (!provider) {
        const err = new Error(`${args.fieldName} must be a carrier provider in the same company`);
        err.statusCode = 400;
        throw err;
    }
    return provider;
...
function normalizeRuleInput(input) {
    ...
    if (!Number.isFinite(priority)) {
        const err = new Error("priority must be a number");
        err.statusCode = 400;
        throw err;
    }
    ...
    if (Number.isNaN(minWeightKg) || Number.isNaN(maxWeightKg)) {
        const err = new Error("weight bounds must be numeric");
        err.statusCode = 400;
        throw err;
    }
    if (minWeightKg != null && minWeightKg < 0) {
        const err = new Error("minWeightKg must be >= 0");
        err.statusCode = 400;
        throw err;
    }
    if (maxWeightKg != null && maxWeightKg <= 0) {
        const err = new Error("maxWeightKg must be > 0");
        err.statusCode = 400;
        throw err;
    }
    if (minWeightKg != null && maxWeightKg != null && maxWeightKg < minWeightKg) {
        const err = new Error("maxWeightKg must be greater than or equal to minWeightKg");
        err.statusCode = 400;
        throw err;
    }
    const legSequence = intOrNull(input.legSequence);
    if (Number.isNaN(legSequence) || (legSequence != null && legSequence <= 0)) {
        const err = new Error("legSequence must be a positive integer");
        err.statusCode = 400;
        throw err;
    }
    return {
...
async function listCarrierRoutingRulesForActor(args) {
    await (0, identity_access_1.authorize)(args.user, "integration.routing.read");
...
async function createCarrierRoutingRuleForActor(args) {
    ...
    const row = await db.carrierRoutingRule.create({
        data: {
            ...
            minWeightKg: normalized.minWeightKg == null ? null : new client_1.Prisma.Decimal(normalized.minWeightKg),
            maxWeightKg: normalized.maxWeightKg == null ? null : new client_1.Prisma.Decimal(normalized.maxWeightKg),
            legSequence: normalized.legSequence,
            conditionsJson: normalized.conditionsJson === undefined
                ? undefined
...
async function updateCarrierRoutingRuleForActor(args) {
    const existing = await getRuleAccessibleOrThrow(args.user, args.ruleId, "integration.routing.manage");
    const merged = {
        ...
        fallbackProviderId: args.input.fallbackProviderId === undefined
            ? existing.fallbackProviderId
            : args.input.fallbackProviderId,
        routeTemplateId: args.input.routeTemplateId === undefined
            ? existing.routeTemplateId
            : args.input.routeTemplateId,
        routeTemplateLegId: args.input.routeTemplateLegId === undefined
        ...
        transportMode: args.input.transportMode === undefined ? existing.transportMode : args.input.transportMode,
        originCountryCode: args.input.originCountryCode === undefined
            ? existing.originCountryCode
            : args.input.originCountryCode,
        destinationCountryCode: args.input.destinationCountryCode === undefined
            ? existing.destinationCountryCode
            : args.input.destinationCountryCode,
        minWeightKg: args.input.minWeightKg === undefined
            ? existing.minWeightKg == null
                ? null
                : Number(existing.minWeightKg)
            : args.input.minWeightKg,
        maxWeightKg: args.input.maxWeightKg === undefined
        ...
        conditionsJson: args.input.conditionsJson === undefined ? existing.conditionsJson : args.input.conditionsJson,
    };
...
async function deleteCarrierRoutingRuleForActor(args) {
    const existing = await getRuleAccessibleOrThrow(args.user, args.ruleId, "integration.routing.manage");
    await db.carrierRoutingRule.delete({ where: { id: existing.id } });
    return { deleted: true, id: existing.id };
}
function resolveLegOriginCountry(leg) {
    return (normalizeCountryCode(leg.fromCountry) ||
        normalizeCountryCode(leg.order?.senderAddressObj?.country));
}
function resolveLegDestinationCountry(leg) {
    return (normalizeCountryCode(leg.toCountry) ||
        normalizeCountryCode(leg.order?.receiverAddressObj?.country));
}
async function resolveCarrierRoutingRuleForOrderLeg(args) {
    ...
    if (orderOrgIds.length > 0 && !orderOrgIds.includes(companyId))
        return null;
