...
exports.listRouteTemplatesForActor = listRouteTemplatesForActor;
exports.getRouteTemplateForActor = getRouteTemplateForActor;
exports.createRouteTemplateForActor = createRouteTemplateForActor;
exports.updateRouteTemplateForActor = updateRouteTemplateForActor;
exports.deleteRouteTemplateForActor = deleteRouteTemplateForActor;
...
function normalizeLegCode(value) {
    const normalized = String(value || "")
        .trim()
        .replace(/[-\s]+/g, "_")
        .replace(/[^A-Za-z0-9_]/g, "")
...
async function getTemplateAccessibleOrThrow(user, routeTemplateId, permission) {
    await (0, identity_access_1.authorize)(user, permission);
    const scopedIds = await listAccessibleCompanyIds(user);
    ...
    if (!routeTemplate) {
        const err = new Error("Route template not found");
        err.statusCode = 404;
        throw err;
    }
    if (scopedIds !== null && !scopedIds.includes(routeTemplate.companyId)) {
        const err = new Error("Forbidden for this route template");
        err.statusCode = 403;
        throw err;
    }
...
function normalizeLegInput(leg) {
    const sequence = intOrDefault(leg.sequence, NaN);
    if (!Number.isFinite(sequence) || sequence <= 0) {
        const err = new Error("route template leg sequence must be a positive integer");
        err.statusCode = 400;
        throw err;
    }
    const legCode = normalizeLegCode(leg.legCode);
    if (!legCode) {
        const err = new Error("route template leg code is required");
        err.statusCode = 400;
        throw err;
    }
    return {
...
function normalizeTemplateInput(input) {
    ...
    if (!Number.isFinite(priority)) {
        const err = new Error("priority must be an integer");
        err.statusCode = 400;
        throw err;
    }
    if (!Array.isArray(input.legs) || input.legs.length === 0) {
        const err = new Error("route template requires at least one leg");
        err.statusCode = 400;
        throw err;
    }
    ...
    for (let index = 0; index < legs.length; index += 1) {
        ...
        if (seenSequences.has(leg.sequence)) {
            const err = new Error(`Duplicate route leg sequence: ${leg.sequence}`);
            err.statusCode = 400;
            throw err;
        }
        seenSequences.add(leg.sequence);
        if (seenCodes.has(leg.legCode)) {
            const err = new Error(`Duplicate route leg code: ${leg.legCode}`);
            err.statusCode = 400;
            throw err;
        }
        seenCodes.add(leg.legCode);
        if (leg.sequence !== index + 1) {
            const err = new Error("Route leg sequence must be continuous and start from 1");
            err.statusCode = 400;
            throw err;
        }
        if (index === 0 && originCountryCode && leg.originCountryCode !== originCountryCode) {
            const err = new Error("First route leg origin must match route originCountryCode");
            err.statusCode = 400;
            throw err;
        }
        if (index === legs.length - 1 &&
            ...
            const err = new Error("Last route leg destination must match route destinationCountryCode");
            err.statusCode = 400;
            throw err;
        }
        if (index > 0) {
            const prev = legs[index - 1];
            if (prev.destinationCountryCode &&
                leg.originCountryCode &&
                prev.destinationCountryCode !== leg.originCountryCode) {
                const err = new Error("Route leg chain is broken: previous destination must equal next origin");
                err.statusCode = 400;
                throw err;
            }
        }
...
async function listRouteTemplatesForActor(args) {
...
async function getRouteTemplateForActor(args) {
    const row = await getTemplateAccessibleOrThrow(args.user, args.routeTemplateId, "integration.routing.read");
    return mapTemplateRow(row);
}
async function createRouteTemplateForActor(args) {
...
async function updateRouteTemplateForActor(args) {
    const existing = await getTemplateAccessibleOrThrow(args.user, args.routeTemplateId, "integration.routing.manage");
    const merged = {
        ...
        transportMode: args.input.transportMode === undefined
        ...
        legs: args.input.legs === undefined
    ...
    const row = await db.$transaction(async (tx) => {
        if (args.input.legs !== undefined) {
            ...
            if (unknownLegId) {
                const err = new Error("route template leg id does not belong to this route template");
                err.statusCode = 400;
                throw err;
            }
            await Promise.all(incomingLegIds.map((id, index) => tx.routeTemplateLeg.update({
                where: { id },
                data: {
                    sequence: -(index + 1),
                    legCode: `__tmp_${id}`,
...
async function deleteRouteTemplateForActor(args) {
    const existing = await getTemplateAccessibleOrThrow(args.user, args.routeTemplateId, "integration.routing.manage");
