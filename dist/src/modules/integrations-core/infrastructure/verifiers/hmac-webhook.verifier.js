...
exports.createHmacWebhookVerifier = createHmacWebhookVerifier;
...
function parsePayload(rawBody) {
    ...
    return (0, querystring_1.parse)(trimmed);
}
...
function normalizeProvidedSignature(rawSignature) {
    ...
    if (v1Token)
        return v1Token.slice(3).trim();
    if (trimmed.startsWith("sha256="))
        return trimmed.slice("sha256=".length).trim();
    return trimmed;
}
function signaturesEqual(expectedHex, providedSignature) {
...
function createHmacWebhookVerifier(args) {
    const providerCode = normalizeProviderCode(args.providerCode);
    const secret = String(args.secret || "");
    const signatureHeaderName = String(args.signatureHeaderName || "").trim().toLowerCase() || "x-signature";
    const timestampHeaderName = String(args.timestampHeaderName || "").trim().toLowerCase() || "x-signature-timestamp";
        ...
        async verifyAndNormalize(input) {
            ...
            if (timestamp) {
                ...
                if (Number.isFinite(epoch)) {
                    const eventMs = epoch > 1000000000000 ? epoch : epoch * 1000;
            ...
            const expectedHex = (0, crypto_1.createHmac)("sha256", secret).update(signedPayload).digest("hex");
            ...
            const eventId = pickString(payload, ["eventId", "event_id", "id", "webhookEventId"]) ??
                (0, crypto_1.createHash)("sha256").update(input.rawBody).digest("hex");
            const eventType = pickString(payload, ["eventType", "event_type", "type"]) ??
                `${providerCode}.event`;
            const occurredAt = toIsoOrNow(pickString(payload, ["occurredAt", "occurred_at", "createdAt", "timestamp"]));
            const companyId = input.companyHintId ??
            ...
            return {
                ...
                data: {
                    ...
                    aggregateType: pickString(payload, ["aggregateType", "aggregate_type", "resource", "entity"]) ??
                        null,
                    aggregateId: pickString(payload, [
                        "aggregateId",
                        "aggregate_id",
                        "resourceId",
                        "resource_id",
                    ...
                    rawBodySha256: (0, crypto_1.createHash)("sha256").update(input.rawBody).digest("hex"),
                },
            };
        },
    };
