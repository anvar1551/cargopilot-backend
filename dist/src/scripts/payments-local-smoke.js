...
function requireBaseEnv() {
    const missing = [];
    if (!TOKEN)
        missing.push("SMOKE_TOKEN");
    if (!COMPANY_ID)
        missing.push("SMOKE_COMPANY_ID");
    if (!ORDER_ID)
        missing.push("SMOKE_ORDER_ID");
    if (missing.length) {
...
async function createIntent(provider) {
    const idempotencyKey = `smoke-${provider.toLowerCase()}-${Date.now()}-${(0, crypto_1.randomUUID)().slice(0, 8)}`;
    const { status, data } = await api("/payments/intents", {
        method: "POST",
        body: JSON.stringify({
...
async function fetchIntent(id) {
    const { status, data } = await api(`/payments/intents/${id}`);
    if (status >= 300) {
...
async function runClick() {
    ...
    const callback = await api("/payments/click/callback", {
        method: "POST",
        auth: false,
...
async function runPayme() {
    ...
    const callback = await api("/payments/payme/callback", {
        method: "POST",
        auth: false,
        headers: {
...
async function runUzum() {
    if (!UZUM_SECRET || !UZUM_SERVICE_ID || !UZUM_AUTH_USER) {
        console.log("[UZUM] skipped: SMOKE_UZUM_SECRET or SMOKE_UZUM_SERVICE_ID or SMOKE_UZUM_AUTH_USER missing");
        return;
    }
    ...
    const callback = await api("/payments/uzum/callback", {
        method: "POST",
        auth: false,
        headers: {
            Authorization: `Basic ${authBasic}`,
...
async function runStripe() {
    ...
    const signature = stripe_1.default.webhooks.generateTestHeaderString({
        payload,
        secret: STRIPE_WEBHOOK_SECRET,
    });
    const callback = await api("/payments/stripe/callback", {
        method: "POST",
        auth: false,
        headers: {
