...
const DEFAULT_RULES = [
    ...
    },
    {
        ...
        source: client_1.SupportTicketSource.driver_app,
        priority: null,
        conditionsJson: {},
        sortOrder: 50,
    },
    {
        ...
        source: client_1.SupportTicketSource.customer_chat,
        priority: null,
        conditionsJson: {},
        sortOrder: 60,
    },
...
const DEFAULT_SLA_POLICIES = [
    { code: "global_normal_24h", name: "Global normal response", queueCode: null, priority: client_1.SupportTicketPriority.normal, targetMinutes: 24 * 60, warningMinutes: 18 * 60 },
    { code: "global_high_8h", name: "Global high response", queueCode: null, priority: client_1.SupportTicketPriority.high, targetMinutes: 8 * 60, warningMinutes: 6 * 60 },
    { code: "global_urgent_2h", name: "Global urgent response", queueCode: null, priority: client_1.SupportTicketPriority.urgent, targetMinutes: 2 * 60, warningMinutes: 90 },
    { code: "finance_high_4h", name: "Finance high response", queueCode: "finance", priority: client_1.SupportTicketPriority.high, targetMinutes: 4 * 60, warningMinutes: 3 * 60 },
    { code: "finance_urgent_1h", name: "Finance urgent response", queueCode: "finance", priority: client_1.SupportTicketPriority.urgent, targetMinutes: 60, warningMinutes: 45 },
    { code: "integrations_high_4h", name: "Integrations high response", queueCode: "integrations", priority: client_1.SupportTicketPriority.high, targetMinutes: 4 * 60, warningMinutes: 3 * 60 },
    { code: "integrations_urgent_1h", name: "Integrations urgent response", queueCode: "integrations", priority: client_1.SupportTicketPriority.urgent, targetMinutes: 60, warningMinutes: 45 },
    { code: "driver_support_high_4h", name: "Driver support high response", queueCode: "driver_support", priority: client_1.SupportTicketPriority.high, targetMinutes: 4 * 60, warningMinutes: 3 * 60 },
    { code: "driver_support_urgent_1h", name: "Driver support urgent response", queueCode: "driver_support", priority: client_1.SupportTicketPriority.urgent, targetMinutes: 60, warningMinutes: 45 },
];
async function resolveCompanyId() {
    const explicitCompanyId = String(process.env.SUPPORT_BOOTSTRAP_COMPANY_ID ?? "").trim();
    if (explicitCompanyId)
        return explicitCompanyId;
    const root = await prismaClient_1.default.organization.findFirst({
    ...
    throw new Error("No active company found. Run bootstrap:erp-access first or set SUPPORT_BOOTSTRAP_COMPANY_ID.");
}
async function main() {
    const companyId = await resolveCompanyId();
    const queuesByCode = new Map();
    for (const queue of DEFAULT_QUEUES) {
        const saved = await prismaClient_1.default.$transaction(async (tx) => {
            if (queue.isDefault) {
    ...
    for (const rule of DEFAULT_RULES) {
        ...
        if (!queue)
            throw new Error(`Queue ${rule.queueCode} was not created`);
        await prismaClient_1.default.supportAssignmentRule.upsert({
    ...
    await prismaClient_1.default.supportAssignmentRule.updateMany({
        where: {
            companyId,
    ...
    for (const policy of DEFAULT_SLA_POLICIES) {
        ...
        await prismaClient_1.default.supportSlaPolicy.upsert({
    ...
    console.log(`[support-bootstrap] done company=${companyId} queues=${DEFAULT_QUEUES.length} rules=${DEFAULT_RULES.length} slaPolicies=${DEFAULT_SLA_POLICIES.length}`);
}
main()
    .catch((err) => {
