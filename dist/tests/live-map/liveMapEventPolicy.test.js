...
const liveMapEventPolicy_1 = require("../../src/modules/live-map-core/application/liveMapEventPolicy");
const tashkentViewport = {
    minLat: 41.1,
    minLng: 68.85,
    maxLat: 41.5,
    maxLng: 69.65,
...
describe("live-map event policy", () => {
    it("allows managers to discover drivers outside the current viewport", () => {
        expect((0, liveMapEventPolicy_1.canDeliverLiveMapEvent)({ actor: manager, event: locationEvent(), viewport: tashkentViewport })).toBe(true);
    });
    it("keeps warehouse streams constrained to their viewport", () => {
        expect((0, liveMapEventPolicy_1.canDeliverLiveMapEvent)({
