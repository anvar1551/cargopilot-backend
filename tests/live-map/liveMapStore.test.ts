...
} from "../../src/modules/live-map-core/infrastructure/liveMapStore";

describe("liveMapStore redis + replay behavior", () => {
  beforeEach(() => {
    jest.clearAllMocks();
    mockWithRedisTimeout.mockImplementation(async (_op: string, work: () => Promise<unknown>) => {
      return await work();
    });
  });
  ...
  it("publishes event locally and replays it from in-memory buffer", async () => {
    ...
    const unsubscribe = subscribeLiveMapEvents(handler);

    const first = {
      type: "driver_location_upsert" as const,
    ...
    await publishLiveMapEvent(first);
    await publishLiveMapEvent(second);

    const firstEventId = handler.mock.calls[0]?.[0]?.id as string | undefined;
    expect(firstEventId).toMatch(/^\d+-\d+$/);

    const replayed = replayLiveMapEventsSince(firstEventId);
  ...
  it("returns unique viewport driver ids from redis geosearch", async () => {
    ...
    const result = await readDriverIdsInViewport({
      minLat: 41,
      minLng: 69,
      maxLat: 42,
      maxLng: 70,
  ...
  it("replays events from redis only for valid stream ids and valid payloads", async () => {
    ...
    const rows = await replayLiveMapEventsFromRedis({
      lastEventId: "100-0",
      limit: 20,
    });
    ...
    const invalid = await replayLiveMapEventsFromRedis({ lastEventId: "invalid" });
