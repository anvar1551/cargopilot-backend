...
type TariffPlanResponse = {
  id: string;
  routeTemplateId?: string | null;
};

type CarrierRoutingRuleResponse = {
  id: string;
  routeTemplateId?: string | null;
...
const cleanupIds = {
  ...
  routeTemplateId: null as string | null,
  carrierRoutingRuleId: null as string | null,
  tariffPlanId: null as string | null,
  originRegionId: null as string | null,
  destinationRegionId: null as string | null,
};
...
async function main() {
  ...
  logStep("Create pricing regions and zone");
  const originRegion = await httpJson<PricingRegionResponse>({
    method: "POST",
  ...
  const destinationRegion = await httpJson<PricingRegionResponse>({
    method: "POST",
    path: "/api/pricing/regions",
    token: admin.token,
    body: {
  ...
  console.log(pretty({ originRegion, destinationRegion, zone: 8 }));

  logStep("Create active fake carrier provider");
  ...
  logStep("Create route template");
  const routeTemplate = await httpJson<RouteTemplateResponse>({
    method: "POST",
  ...
  if (!linehaulLeg) throw new Error("route template linehaul leg missing");
  console.log(pretty(routeTemplate));

  logStep("Create LEG_TRANSIT tariff linked to route template");
  const tariffPlan = await httpJson<TariffPlanResponse>({
    method: "POST",
  ...
  console.log(pretty(tariffPlan));

  logStep("Create carrier rule for exact route template leg");
  const carrierRule = await httpJson<CarrierRoutingRuleResponse>({
    method: "POST",
  ...
  console.log(pretty(carrierRule));

  logStep("Create order using route-template tariff");
  const referenceId = `RT-SMOKE-${runId}`;
  const createdOrder = await httpJson<CreateOrderResponse>({
    method: "POST",
    path: "/api/orders",
    token: admin.token,
    body: {
  ...
  console.log(pretty(createdOrder));

  logStep("Assert generated order legs and carrier booking request");
  ...
  if (!bookedCandidate) throw new Error("linehaul order leg was not generated from route template leg");
  ...
  console.log(pretty({ legs }));

  logStep("Process carrier outbox once");
  ...
  logStep("Send signed carrier status webhook and process canonical event");
  ...
  logStep("Route-template carrier smoke passed");
