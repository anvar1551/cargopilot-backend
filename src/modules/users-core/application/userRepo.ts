...
import { CustomerType, DriverType } from "@prisma/client";
...
export const registerUser = async (args: {
  ...
  email: string;
  password: string;
  role?: ActorRole; // default customer
  ...
  const role = args.role ?? ROLE_CUSTOMER;
...
export const createUserAsManager = async (args: {
  ...
  return prisma.$transaction(async (tx: Prisma.TransactionClient) => {
    ...
    const isCustomer = args.role === ROLE_CUSTOMER;
    ...
    if (
      (args.role === ROLE_WAREHOUSE || args.role === ROLE_DRIVER) &&
      args.warehouseId
    ) {
