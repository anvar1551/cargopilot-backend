...
const createUserByManagerSchema = z
  ...
  .superRefine((v, ctx) => {
    const supportsWarehouse = v.role === ROLE_WAREHOUSE || v.role === ROLE_DRIVER;
    ...
    if (v.role === ROLE_WAREHOUSE && !v.warehouseId) {
      ctx.addIssue({
        code: "custom",
        path: ["warehouseId"],
    ...
    if (v.role !== ROLE_CUSTOMER && v.customerEntityId) {
      ctx.addIssue({
        code: "custom",
    ...
    if (v.driverType && v.role !== ROLE_DRIVER) {
      ctx.addIssue({
        code: "custom",
        path: ["driverType"],
...
const usersFastifyRoutes: FastifyPluginAsync = async (fastify) => {
  fastify.post("/register", async (request, reply) => {
    try {
      ...
      const customerTypeInput =
        typeof body.customerType === "string" ? body.customerType : undefined;
      const customerType =
        customerTypeInput && Object.values(CustomerType).includes(customerTypeInput as CustomerType)
          ? (customerTypeInput as CustomerType)
          : undefined;
  ...
  fastify.post("/refresh", async (request, reply) => {
    try {
      ...
      const statusCode =
        message.includes("refresh token") || message.includes("token")
          ? 401
  ...
  fastify.post("/logout", async (request, reply) => {
    try {
      ...
      return reply.code(err instanceof z.ZodError ? 400 : 400).send({ error: message });
    }
  });
  ...
  fastify.post("/", { preHandler: fastifyAuth({ permission: "users.manage" }) }, async (request, reply) => {
    try {
  ...
  fastify.get("/", { preHandler: fastifyAuth({ permission: "users.manage" }) }, async (request, reply) => {
    try {
      const query = (request.query ?? {}) as Record<string, unknown>;
      const q = typeof query.q === "string" ? query.q : undefined;
      const role =
        typeof query.role === "string" && ACTOR_ROLES.includes(query.role as ActorRole)
          ? (query.role as ActorRole)
          : undefined;
  ...
  fastify.delete("/:id", { preHandler: fastifyAuth({ permission: "users.manage" }) }, async (request, reply) => {
    try {
      if (!request.user?.id) {
        return reply.code(401).send({ error: "Unauthorized" });
      }
