import { getRedisPrefix } from "../../../config/redis";

function parseIntEnv(name: string, fallback: number, min: number) {
...
export const integrationOutboxConfig = {
  ...
  claimProcessingTimeoutMs: parseIntEnv(
  ...
  retryJitterPct: Math.max(
    0,
    Math.min(0.5, Number(process.env.INTEGRATION_OUTBOX_RETRY_JITTER_PCT ?? 0.2)),
  ),
  leaderLockEnabled: parseBoolEnv("INTEGRATION_OUTBOX_LEADER_LOCK_ENABLED", true),
  lockKey:
    process.env.INTEGRATION_OUTBOX_LOCK_KEY ||
    `${getRedisPrefix()}:cp:integrations:outbox:worker:lock`,
  ...
  consumerId:
    process.env.INTEGRATION_OUTBOX_CONSUMER_ID ||
    `${process.env.HOSTNAME || "api"}-${process.pid}`,
  defaultProviderTimeoutMs: parseIntEnv(
    "INTEGRATION_OUTBOX_PROVIDER_TIMEOUT_MS",
    10_000,
    1000,
  ),
} as const;
