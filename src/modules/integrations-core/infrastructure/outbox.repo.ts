...
function mapOutbox(row: any): IntegrationOutboxRecord {
  return {
    id: row.id,
...
async function createAttempt(args: {
  ...
  outcome: "success" | "retry" | "dead_letter" | "failed";
}) {
  await args.tx.integrationDeliveryAttempt.create({
    data: {
      ...
      requestJson: (args.attempt.requestJson ?? null) as any,
      responseJson: (args.attempt.responseJson ?? null) as any,
...
export const integrationOutboxRepository: IntegrationOutboxRepository = {
